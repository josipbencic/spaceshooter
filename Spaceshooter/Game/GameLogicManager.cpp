...
namespace loop_tools {
  ...
  void GLMAN::render() const {
    ...
    for (auto it = weak_enemies.cbegin(); it != weak_enemies.cend(); ++it) {
      if ((*it).alive()) {
        (*it).render();
        if (health_bars) {
          (*it).render_hp_bars();
    ...
    for (auto it = strong_enemies.cbegin(); it != strong_enemies.cend(); ++it) {
      if ((*it).alive()) {
        (*it).render();
        if (health_bars) {
          (*it).render_hp_bars();
  ...
  inline void missile_comet_collision(
    ...
    for (typename SMP<Missile_>::iterator it = missiles.begin();
      ...
      for (auto it2 = comets.begin(); it2 != comets.end(); ++it2) {
  ...
  void GLMAN::generate_game_objects() {
    ...
    static double last_health = al_get_time();
    ...
    if ((time - last_health) > 2.0 && is_between(random_number, 25, 27)) {
