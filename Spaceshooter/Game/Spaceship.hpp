...
namespace sprites {
	...
	public:
		...
		void reset_power_up_timer(double duration) {
			power_up_time = duration;
		}
			
		void set_current_missile_type(MissileType type) {
			missile_type_ = type;
		}

		MissileType missile_type() const {
		...
		float get_bound_circle_radius() const {
